import { completeScrambleGameBackend } from "@/app/utils/dbFunctions";
import logger from "@/app/utils/logger";
import { jsonResponse } from "@/app/utils/response";
import { JScrambleGame } from "@/app/utils/types";
import {
    createActionHeaders,
...
import { StatusCodes } from "http-status-codes";
  ...
  export const POST = async (req: Request) => {
    try {
		...
		const ScrambleGameData: JScrambleGame = {
			id: gameID,
			player2Account: account.toString()
		};
		const success = await completeScrambleGameBackend(ScrambleGameData);

		if(success==false) {
			throw new Error("Error in joining game!!");
		}
  
		const message = `You’ve been challenged! Now it's your turn to test your scramble skills and beat the score.\n
Click here to play: https://scramble.nixblack.site/game/${gameID}`
