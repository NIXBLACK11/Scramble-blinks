...
import { getScrambleGameById } from "@/app/utils/dbFunctions";
import { FullScrambleGame } from "@/app/utils/types";
  ...
  export const GET = async (req: Request) => {
    try {
      logger.info("GET request received for join Scramble");
      ...
      const fullGame: FullScrambleGame | null = await getScrambleGameById(gameID);
      ...
      if(fullGame.player1Joined==false) {
        const payload: ActionGetResponse = {
          title: "Player 1 has not played yet!",
          icon: new URL("/back.gif", requestUrl.origin).toString(),
          type: "action",
          description: `Player 1 has not played their chance yet!!`,
          label: "Initial score not set",
          links: { actions: [] },
        };
        return jsonResponse(payload, StatusCodes.OK, headers);
      }

      if(fullGame.player1Joined==true && fullGame.player2Joined==true && fullGame.player1Score!=-1 && fullGame.player2Score!=-1) {
        const winnerScore = (fullGame.winner==="1") ? fullGame.player1Score : fullGame.player2Score;
        const payload: ActionGetResponse = {
          title: "This game has been completed!",
          icon: new URL("/back.gif", requestUrl.origin).toString(),
          type: "action",
          description: `Player ${fullGame.winner} has won the game with a score of ${winnerScore}`,
      ...
      const payload: ActionGetResponse = {
        title: "Someone wants to test your Speed😈!",
        icon: new URL("/back.gif", requestUrl.origin).toString(),
        type: "action",
        description: `Player 1 has set a wager of ${fullGame.wager}SOL and completed the scramble words challenge! Now it’s your turn to beat their score and claim the win. Can you solve the challenge faster?`,
        label: "Join the scramble challenge!!",
        links: { actions },
      };
  ...
  export const POST = async (req: Request) => {
    try {
        logger.info("POST request received for join Scramble");
